    ...
    public class RibbonManager
    {
        public static void CreateRibbonTab(UIControlledApplication application)
        {
            try
            {
                ...
                // Получаем путь к папке с DLL файлами (где находится текущая сборка)
                string assemblyPath = Assembly.GetExecutingAssembly().Location;
                string assemblyDir = Path.GetDirectoryName(assemblyPath);

                // Получаем или создаем панель DataHub
                // ПРИМЕЧАНИЕ: Панель "Настройки" уже должна быть создана через PluginLoader
                // (AuthApplication создает её первой), поэтому мы создаем только DataHub
                RibbonPanel dataPanel = GetOrCreatePanel(application, tabName, "DataHub");

                // Создаем кнопку DataViewer
                PushButtonData dataViewerButtonData = new PushButtonData(
                    "DataViewer",
                    "DataViewer",
                    assemblyPath,
                    "RevitExporterAddin.ExportCommand");

                dataViewerButtonData.ToolTip = "Управление проектами и экспорт данных из Revit в CSV";
                dataViewerButtonData.LongDescription = "Открывает интерфейс для управления проектами экспорта данных из Revit";
                
                // Загружаем иконки из файлов
                dataViewerButtonData.Image = LoadIconFromFile("dataviewer_16.png");
                dataViewerButtonData.LargeImage = LoadIconFromFile("dataviewer_32.png");
                
                // Устанавливаем контекст команды - всегда доступна
                dataViewerButtonData.AvailabilityClassName = "RevitExporterAddin.AlwaysAvailable";

                // Добавляем кнопку DataViewer в группу
                PushButton dataViewerButton = dataPanel.AddItem(dataViewerButtonData) as PushButton;
                
                // Принудительно устанавливаем свойства кнопки
                if (dataViewerButton != null)
                {
                    dataViewerButton.Visible = true;
